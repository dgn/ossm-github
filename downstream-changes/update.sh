...
BRANCHES=${BRANCHES:-master release-1.23 release-1.24}
...
function renderMarkdownTableFromYAML() {
  ...
  echo "| Commit SHA | Title | Upstream PR | Comment | Date | Author |"
  echo "| --- | --- | --- | --- | --- |--- |"
  while IFS=$'\t' read -r sha title upstreamPR comment date author _; do
    ...
    echo "| [${sha}](${COMMIT_BASE_URL}${sha}) | `renderTitle "${title}"` | ${upstreamPR} | ${comment} | ${date} | ${author} |"
  done < <(yq e '.commits[] | [.sha, .title, .upstreamPR, .comment, .date, .author] | @tsv' ${yaml_file})
}
